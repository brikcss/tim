...
describe('utils/merge', () => {
	...
	it('should merge two arrays, even if they are empty', () => {
		tim.assert.deepEqual(tim.utils.merge([], ['one', 'two']), ['one', 'two']);
	});
	...
	it('should clone objects when the first argument is empty', () => {
		...
		let merged = tim.utils.merge({}, brik, { options: child });
		tim.assert.deepEqual(merged, {
			options: {
				files: '/this/path',
				root: '/another/path'
			}
		});
		tim.assert.deepEqual(brik, { options: { files: ['/this/path/one', '/this/path/two'] } });
	});
});
