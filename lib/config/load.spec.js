...
describe('config/load', () => {
	...
	it.skip('loads a config file which returns a function', () => {
		const startPath = tim.path.join(configsPath, 'function/brik.config.js');
	...
	it.skip('extend config file with JS, JSON, and YAML configs', () => {
		...
		return tim.config.load({ startPath, extend: '_brik' }).then(result => {
			tim.assert.ok(result.meta.success);
			tim.assert.equal(result.meta.startPath, startPath);
			tim.assert.equal(result.meta.entry, tim.path.join(startPath, '.brikrc.js'));
			tim.assert.deepEqual(result.data, {
				name: 'test.js',
				_brik: {
			...
			tim.assert.ok(
				result.meta.extends.indexOf(tim.path.join(configsPath, 'json/.brikrc')) > -1,
				'result.meta.extends contains `json/.brikrc`'
			);
			tim.assert.ok(
				result.meta.extends.indexOf(tim.path.join(configsPath, 'yaml/.brikrc')) > -1,
				'result.meta.extends contains `yaml/.brikrc`'
			);
			tim.assert.ok(
				result.meta.extends.indexOf(tim.path.join(configsPath, 'jsonExtended/brik.config.js')) > -1,
				'result.meta.extends contains `jsonExtended/brik.config.js`'
			);
			tim.assert.ok(
				result.meta.extends.indexOf(tim.path.join(configsPath, 'yamlExtended/brik.config.js')) > -1,
				'result.meta.extends contains `yamlExtended/brik.config.js`'
			);
			tim.assert.ok(
				result.meta.extends.indexOf(tim.path.join(configsPath, 'function/brik.config.js')) > -1,
				'result.meta.extends contains `function/brik.config.js`'
			);
