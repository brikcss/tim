		...
		tim.config
			...
			.then(config => {
				...
				options.files.forEach(brikFiles => {
					...
					// Compile each brik.
					bootPromises.push(compileABrik(brikFiles, options, data));
				});
					...
					(result, brik) => {
						...
						brik.files.forEach(file => {
							...
							if (options.relativePaths) {
								...
								brik.startPath = tim.path.relative(options.cwd, brik.startPath);
								file.in = tim.path.relative(options.cwd, file.in);
								file.out = tim.path.relative(options.cwd, file.out);
								file.startPath = tim.path.relative(options.cwd, file.startPath);
							}
 ...
 *  @param   {object}  [options]  User options. Consists of node/cli options merged with `_brik` property of main config.
 *  @param   {object}  [data]  Data passed from export function.
 *  @return  {promise}  Promise to return array of file objects.
 */
function compileABrik(brikFiles, options = {}, data = {}) {
	// Create brik object, which is what will be returned.
	let brik = {
		root: options.root || options.on.getBrikRoot(brikFiles, { data, options, tim }),
		startPath: brikFiles,
		files: [],
		options: options
	};
	...
	return (
		...
		tim.config
			...
			.then(brikDefaultConfig => {
				// Cache brik-specific options.
				const brikDefaults = brikDefaultConfig.data._brik || {};
				let brikConfig = {};
				brik.name = brikDefaults.name || tim.path.relative(options.cwd, brik.root);
...
function requireExportFile(file, { data = {}, options = {}, stringify = true }) {
	...
	if (typeof content === 'function') {
		content = content(data, options, tim);
	}
...
function mergeOptions(options = {}, config = {}) {
	const configOptions = config.data._brik || {};
	const userOptions = tim.utils.merge(options, configOptions);
	const defaultOptions = require('../../.brikcssrc-defaults')(userOptions)._brik;
