...
module.exports = (options = {}, data = {}) => {
	...
	return (
		tim.config
			...
			.then(brik => {
				brik = tim.utils.merge(brik, { options, data });
				return buildABrik(brik, { userBrik: brik });
			})
			// Parse results.
			.then(results => {
...
function buildABrik(brik = {}, cache = {}) {
	...
	if (!cache.entryLoaded) {
		...
		brik.options = tim.utils.merge(require('../../.brikcssrc-defaults')(brik.options)._brik, brik.options);
	...
	return new Promise((resolve, reject) => {
		...
		if (file.isJsExport) {
			...
			file.content = tim.ejs.render(
				...
				{ options: brik.options, data: brik.data, meta: brik.meta, tim },
				brik.options.ejs || {}
			);
		}
 ...
 *  @param   {object}  [brik]  Brik options to compile a file with. Typically comes from `<config file>._brik[<name property in .brikrc>]`.
