...
describe('boot/index', () => {
	...
	describe('paths', () => {
		...
		const errorMsg =
			'Error: options.files OR options.extends is required and must be a string or Array of strings.';

		afterEach('clean up', () => {
			tim.rm.sync(outputFile);
		});

		it('returns an empty result if no files are passed', () => {
			return tim.boot().then(result => {
				tim.assert.ok(result instanceof Error);
				tim.assert.equal(result, errorMsg);
			});
		});
	...
	describe('options.config', () => {
		...
		it('runs with options.config as a file', () => {
			return tim
				.boot({
					config: {
						entry: 'fixtures/configs/brik.config.js'
					},
					files: 'fixtures/boot/test1/test.md'
				})
				.then(result => {
	...
	describe.skip('briks', () => {
		it('boots multiple briks into one', () => {
			return tim
				.boot(
					{
				...
				.then(result => {
					...
					tim.assert.deepEqual(tim.fs.readJsonSync('.temp/multiple/package.json', 'utf8'), {
						scripts: {
							'js:lint': 'node_modules/.bin/eslint **/*{.js,.xjs,.xjson} --format=stylish',
							'js:lint:watch':
								"chokidar **/*{.js,.xjs,.xjson} --command 'node_modules/.bin/eslint {path} --format=stylish && npm run postjs:lint'",
							'css:lint': 'node_modules/.bin/stylelint **/*{.css} --format=stylish',
							'css:lint:watch':
								"chokidar **/*{.css} --command 'node_modules/.bin/eslint {path} --format=stylish && npm run postjs:lint'",
							gcz:
						...
						devDependencies: {
							...
							'@brikcss/stylelint-config-css': '0.0.5',
		...
		it('extends multiple briks into one', () => {
			return tim
				.boot(
					{
						config: {
							entry: 'fixtures/boot/extends/.brikcssrc.js',
							extend: true
						}
					},
					{
				...
				.then(() => {
					...
					tim.assert.equal(tim.fs.pathExistsSync('.temp/extends/.brikrc.js'), false);
					filesEqualToTheirSource.forEach(filepath => {
						tim.assert.equal(
							tim.fs.readFileSync(
								tim.path.join(
							...
							tim.fs.readFileSync(tim.path.join('fixtures/boot/extends', filepath), 'utf8')
						);
					});
					tim.assert.equal(
						tim.fs.readFileSync('.temp/extends/.eslintignore', 'utf8'),
						'## -------------------------------------------------------------------------------------------------\n# .eslintignore\n# -------------\n# @author  brikcss <https://github.com/brikcss>\n# @description  Files to ignore by [eslint](https://eslint.org/docs/user-guide/configuring#ignoring-files-and-directories).\n## -------------------------------------------------------------------------------------------------\n\ntest/**/*.js\n'
					);
					tim.assert.deepEqual(tim.fs.readJsonSync('.temp/extends/package.json', 'utf8'), {
						scripts: {
							gcz:
								'echo "See the "git commit policy" in CONTRIBUTING.md for details on how to write a valid commit message.\n" && git-cz',
							commitmsg: 'commitlint -e $GIT_PARAMS',
		...
		it('extends multiple briks with a function', () => {
			return tim
				.boot(
					{
						config: {
							entry: 'fixtures/boot/extends/brikcss.config.js',
							extend: true
						}
					},
					{
	...
	describe('git', function() {
		...
		it('boots a simple git repo', () => {
			const output = '.temp/github-simple';
			return tim
				.boot(
					{ files: 'gh@brikcss/boot-test', output },
					{
						headline: 'Tim I am.',
						holla: 'Hello world!!',
						greeting: 'Hello there. This is your warm greeting.'
				...
				.then(result => {
					tim.assert.deepEqual(tim.fs.readJsonSync(output + '/package.json', 'utf8'), {
						scripts: {
							test: 'echo "I am a test. Hello world!!!"'
						}
					});
					tim.assert.equal(tim.fs.readFileSync(output + '/README.md', 'utf8'), '# boot-test\n\nTim I am.\n');
					tim.assert.equal(
						tim.fs.readFileSync(output + '/TODO.md', 'utf8'),
						'# Test git repo\n\nHello there. This is your warm greeting.'
					);
					return tim.globby(output).then(actualFilepaths => {
						tim.assert.deepEqual(
							actualFilepaths.sort(),
							[output + '/README.md', output + '/TODO.md', output + '/package.json'].sort(),
							'only non-skipped files should be compiled'
						);
						return result;
					});
		...
		it.skip('boots a git repo and local directory together', () => {
			return tim
				.boot(
					{
