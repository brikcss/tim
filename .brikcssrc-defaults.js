...
module.exports = userOptions => {
	return {
		_tim: {
			// @property  {object}  [boot]  Settings for Tim's `boot` function.
			boot: {
				// @property  {string|array}  files  String or array of "briks" to compile. Each "brik" can be a glob, directory, file, or array of globs, files, and/or directories.
				files: [],
				...
				config: {
					...
					entry: undefined,
					// @property  {boolean|string}  [extend]  Whether to extend config with files in the `extends` property. A string will look for `extends` on this custom property (i.e., '_tim' would look for `<config>._tim.extend`).
					extend: false
				},
				...
				on: {
					...
					getBrikRoot: (brikFilepaths, { options, data, tim }) => {
						return tim.utils.getRoot(
							tim.path.resolve(
								options.cwd,
								brikFilepaths instanceof Array ? brikFilepaths[0] : brikFilepaths
							)
						);
					},
			...
			// @property  {object}  [briks]  Configuration for individual briks. For example, briks: {'@brikcss/test', {...}} would provide individual configuration for the brik named '@brikcss/test'.
			briks: {}
		}
	};
};
